...
def read_arguments():
    parser = argparse.ArgumentParser(description="takes segemehl SAM output and merges split reads into one SAM line in addition some entries are tinkered with")
...
def init_metrics():
    ...
    indict =     OrderedDict()
    split_types = ['not_same_chr', 'not_same_strand','max_dist_breached','fragment_overlap', 'order_switch', 'OK']
...
def process_split_reads(aln_dict):
    ...
    for XI, template  in aln_dict.iteritems():
        for i in [0,1]:
    ...
    for i in suitable:
        ...
        if args.read_type == 'paired':
            ...
            if i in single_read_placement.keys():
                ...
                (template[0][0],template[1][0] ) = samdict_set_pe_mate(samdicts[0],samdicts[1])
                aln_dict[i] = template
    ...
    if info['type'] != 'OK':
     ...
     #       pp.pprint(i)
      #      pp.pprint(samdict)
            ...
            new_samdict['cigar'] =  ''.join([new_samdict['cigar'], str(diff), 'N', samdict['cigar']])
...
def samdict_set_pe_mate(samdict_A, samdict_B):
    ...
    if (samdict_A['rname'] == samdict_B['rname']):
        ...
        if (testBit(samdict_A['flag'],4) > 0 and testBit(samdict_A['flag'],5) == 0):
...
def get_genomic_sequence_length(cigar):
    ...
    tlist = re.findall('\d+\D', cigar, flags=0)
    for i in tlist:
        cigar_type = i[-1:]
        count = int(i[:-1])
...
def first_or_second_read(flag):
    ...
    if (fst > 0 and snd == 0):
        ...
        raise StandardError( 'Not unique flag assignment fst: {0} snd: {1}'.format(fst, snd))
